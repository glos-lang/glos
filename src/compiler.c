...
static QbeNode *compile_fn(Compiler *c, NodeFn *fn, QbeNode **export_qbe) {
    ...
    QbeFn *fn_save = c->fn;
    size_t defers_start_save = c->defers_start;
...
static QbeNode *compile_expr(Compiler *c, Node *n, bool ref) {
    ...
    case NODE_INDEX: {
        ...
        if (index->ranged) {
            ...
            // Check if bounds are ascending
            {
                ...
                QbeNode *check = qbe_build_binary(c->qbe, c->fn, QBE_BINARY_ULE, qbe_type_basic(QBE_TYPE_I8), from, to);
    ...
    case NODE_MEMBER: {
        ...
        if (member->lhs->type.kind == TYPE_ARRAY) {
            if (n->token.as.integer) {
                return qbe_atom_int(c->qbe, n->type.qbe.kind, member->lhs->type.spec_count);
            } else {
                return member->lhs_qbe;
            }
        }
        ...
        if (!ref) {
            member->lhs_qbe = qbe_build_load(c->qbe, c->fn, member->lhs_qbe, n->type.qbe, type_is_signed(n->type));
        }
...
static void compile_stmt(Compiler *c, Node *n) {
    ...
    case NODE_DEFER:
        da_push(&c->defers, ((NodeDefer *) n)->stmt);
        break;

    case NODE_RETURN: {
        NodeReturn *ret = (NodeReturn *) n;

        QbeNode *value = compile_expr(c, ret->value, false);
        if (ret->value && ret->value->type.kind == TYPE_UNIT) {
            value = NULL;
        }

        compile_defers(c, c->defers_start, false);
        qbe_build_return(c->qbe, c->fn, value);
        qbe_build_block(c->qbe, c->fn, qbe_block_new(c->qbe));
    } break;

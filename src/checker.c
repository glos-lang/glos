...
void check_nodes(Compiler *c) {
    ...
    // Define the methods
    {
        for (size_t i = 0; i < c->methods_list.count; i++) {
            ...
            if (fn->value_directives.head && !receiver_type.ref) {
                error_token(
                    EK_ERROR,
                    fn->value_directives.head->token,
                    "This iterator overload has value semantics, yet the receiver is not a typed pointer");

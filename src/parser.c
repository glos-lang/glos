...
static void ensure_const_expr(Node *n) {
    ...
    switch (n->kind) {
    case NODE_ATOM:
        if (n->token.kind == TOKEN_NIL) {
            error_full(ERROR, n->token.pos, "Unexpected 'nil' in constant expression");
            exit(1);
        }
        break;
...
static Node *parse_expr(Parser *p, Power mbp, ParseFlags flags) {
    ...
    case TOKEN_NIL:
        if (flags & PF_CONSTANT_EXPR) {
            error_full(ERROR, token.pos, "Unexpected 'nil' in constant expression");
            exit(1);
        }
...
static Node *parse_stmt(Parser *p) {
    ...
    case TOKEN_TYPE: {
        ...
        if (lexer_read(&p->lexer, TOKEN_LBRACKET)) {
            while (true) {
    ...
    case TOKEN_STRUCT: {
        ...
        token = lexer_expect(&p->lexer, TOKEN_LBRACKET, TOKEN_LBRACE);
        if (token.kind == TOKEN_LBRACKET) {
            while (true) {
...
static Node *parse_fn(Parser *p, Token token, bool will_be_method) {
    ...
    token = lexer_expect(&p->lexer, TOKEN_LPAREN, TOKEN_LBRACKET);
    if (token.kind == TOKEN_LBRACKET) {
        if (p->in_extern) {
